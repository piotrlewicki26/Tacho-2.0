...
-- ─────────────────────────────────────────────
-- Driver Border Crossings (event-level source of truth)
-- One row = one border crossing event parsed from driver card.
-- ─────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS `driver_border_crossings` (
  ...
  `crossing_tmin`  SMALLINT UNSIGNED NOT NULL
    COMMENT 'Minute of day (0..1439) in UTC timeline basis',
  `crossing_ts`    INT UNSIGNED DEFAULT NULL
    COMMENT 'Optional absolute crossing timestamp',
...
-- ─────────────────────────────────────────────
-- Vehicle Activity Calendar
-- Continuous per-vehicle per-day distance calendar built from every
-- uploaded vehicle DDD (VU) file.  Keyed by (vehicle_id, date).
-- ─────────────────────────────────────────────
